...
public interface IBillerService
{
    ...
    ErrorOr<List<BillerDataTransferObject>> GetBillerDirectory();
    ...
    ErrorOr<List<BillerDataTransferObject>> SearchBillers(string searchTerm, string? category = null);

    /// <summary>Returns all billers saved by the specified user.</summary>
    /// <param name="userId">The user identifier.</param>
    /// <returns>The list of saved biller DTOs.</returns>
    ErrorOr<List<SavedBillerDataTransferObject>> GetSavedBillers(int userId);
    ...
    ErrorOr<SavedBillerDataTransferObject> SaveBiller(int userId, SaveBillerRequest request);
    ...
    ErrorOr<Success> RemoveSavedBiller(int userId, int savedBillerId);
}