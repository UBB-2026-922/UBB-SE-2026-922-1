...
internal static partial class ApplicationLogMessages
{
    ...
    [LoggerMessage(EventId = 2043, Level = LogLevel.Warning, Message = "Dashboard fetch failed: user {UserId} not found.")]
    internal static partial void DashboardUserNotFound(this ILogger logger, int userId);

    [LoggerMessage(EventId = 2044, Level = LogLevel.Error, Message = "Failed to fetch cards for user {UserId}: {Error}")]
    internal static partial void DashboardFetchCardsFailed(this ILogger logger, int userId, string error);

    [LoggerMessage(EventId = 2045, Level = LogLevel.Error, Message = "Failed to fetch notification count for user {UserId}: {Error}")]
    internal static partial void DashboardFetchNotificationCountFailed(this ILogger logger, int userId, string error);

    [LoggerMessage(EventId = 2046, Level = LogLevel.Error, Message = "Failed to fetch accounts for user {UserId}: {Error}")]
    internal static partial void DashboardFetchAccountsFailed(this ILogger logger, int userId, string error);

    [LoggerMessage(EventId = 2047, Level = LogLevel.Error, Message = "Failed to fetch transactions for account {AccountId}: {Error}")]
    internal static partial void DashboardFetchTransactionsFailed(this ILogger logger, int accountId, string error);
