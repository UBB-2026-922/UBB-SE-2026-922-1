...
public class AuthenticationService : IAuthenticationService
{
    ...
    public ErrorOr<Success> Register(RegisterRequest request)
    {
        ...
        if (!_authenticationRepository.FindUserByEmail(request.Email).IsError)
        {
        ...
        if (_authenticationRepository.CreateUser(newUserResult.Value).IsError)
        {
    ...
    private ErrorOr<LoginSuccess> CompleteLogin(User user)
    {
        ...
        if (_authenticationRepository.CreateSession(user.Id, token, null, null, null).IsError)
        {
